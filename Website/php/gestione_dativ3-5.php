...
$aggregates = htmlspecialchars($_GET["aggregates"]);
...
for($i=0;$i<count($macaddr);$i++){
	...
	$m=$macaddr[$i];
	$result = mysqli_query($conn, "INSERT INTO `logv3`(`Id`, `aggregated` , `Timestamp` , `delta_reali` , `delta_random` , `macaddr`) VALUES ('".$id."','".$aggr."','".$t."','".$dr."','".$dran."','".$m."')") or die(mysqli_error($conn));
