...
def Compara( mac1 , macaddr1 , mac2 , macaddr2 ): #mac1=>set1 mac2=>set2
	...
	elif CalcDist(mac1['log'][-1]['seq'] , mac2['log'][0]['seq']) < 25 and abs(mac1['log'][-1]['time']- mac2['log'][0]['time']) < 15:
		#print "controllo mac1log-1-mac2log0 sq e time"				
		Aggrega(mac1 , macaddr1 , mac2 , macaddr2)	
		return True;

	elif CalcDist(mac1['log'][0]['seq'] , mac2['log'][-1]['seq']) < 25 and abs(mac1['log'][0]['time']- mac2['log'][-1]['time']) < 15:
	...
	elif abs(mac1['log'][0]['time']- mac2['log'][-1]['time']) < 7  and abs(mac1['log'][0]['pow']- mac2['log'][-1]['pow']) < 10  :
	...
	for k in SigList.keys():
		for q in SigList[k].keys():
			if SigList[k][q]['RandomMac']==True:
				if abs(SigList[k][q]['log'][-1]['time'] - time.time()) > 35:
...
def PacketHandler(pkt) :
    ...
    #probe request e controllo che la potenza sia > -99 => -(256-ord(pkt.pkt.ded[-4:-3])=potenza=>RSSI)
		if pkt.type == 0 and pkt.subtype == 4 and pkt.dBm_AntSignal > -99 :
			...
			print "Trovato ", pkt.addr2, "potenza:",pkt.dBm_AntSignal 
