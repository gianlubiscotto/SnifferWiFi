...
#dizionario con tutti gli oui registrati. Aggiornato al 13 agosto 2019
oui_list={}
	...
	for k in SigList.keys():
		for q in SigList[k].keys():
			if SigList[k][q]['RandomMac']==True:
				if abs(SigList[k][q]['log'][-1]['time'] - time.time()) > 30: #edit: era 20
					SigList[k][q]['canc'] = True
					print "cancello random per vecchiaia",q
			else:
				if abs(SigList[k][q]['log'][-1]['time']-time.time()) > 60: #edit: era 30
					SigList[k][q]['canc']=True
