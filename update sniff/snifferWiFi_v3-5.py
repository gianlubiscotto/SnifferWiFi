...
user="cinqueterre",
passwd="5T3rr3",
database="my_sniffer5terre"
)

myId = 1
snifferId = 3 #Vernazza
...
#controlla se i due mac siano da aggregare
def Compara( mac1 , macaddr1 , mac2 , macaddr2 ): #mac1=>set1 mac2=>set2
	#print "Comparo: diff",mac1['log'][-1]['seq']-mac2['log'][-1]['seq']
	...
	if mac1['uuid'] != '' and mac1['uuid'] == mac2['uuid']:
		...
		Aggrega(mac1 , macaddr1 , mac2 , macaddr2) #segno che quello più vecchio è da cancellare
		return True;
	elif mac1['log'][-1]['seq'] > 25 and mac2['log'][0]['seq'] > 25 or mac1['log'][0]['seq'] > 25 and mac2['log'][-1]['seq']:
		if CalcDist(mac1['log'][-1]['seq'] , mac2['log'][0]['seq']) <= 40:
			#print "controllo mac1log-1-mac2log0 sq"			
			Aggrega(mac1 , macaddr1 , mac2 , macaddr2)	
			return True;

		elif CalcDist(mac1['log'][0]['seq'] , mac2['log'][-1]['seq']) <= 40:
			#print "controllo mac1log0-mac2log-1 sq"						
			Aggrega(mac1 , macaddr1 , mac2 , macaddr2)	
			return True;

		elif CalcDist(mac1['log'][0]['seq'] , mac2['log'][-1]['seq']) <= 60 and abs(mac1['log'][0]['pow']- mac2['log'][-1]['pow']) < 10  :
			#print "controllo sn e potenza"
			Aggrega(mac1 , macaddr1 , mac2 , macaddr2)
			return True;
		
		elif CalcDist(mac1['log'][-1]['seq'] , mac2['log'][0]['seq']) < 60 and abs(mac1['log'][-1]['pow']- mac2['log'][0]['pow']) < 10  :

			#print "controllo sn e potenza"
			Aggrega(mac1 , macaddr1 , mac2 , macaddr2)
			return True;	
	
	elif abs(mac1['log'][0]['time']- mac2['log'][-1]['time']) < 21 and abs(mac1['log'][0]['pow']- mac2['log'][-1]['pow']) < 10  :
	#il time del primo log del mac i e il time dell'ultimo log del mac j siano distanti meno di 20 secondi e che le due potenze differiscano poco
		#print "controllo mac1log0-mac2log-1 e potenza"
		Aggrega(mac1 , macaddr1 , mac2 , macaddr2)
		return True;
	
	elif abs(mac1['log'][-1]['time']- mac2['log'][0]['time']) < 21 and abs(mac1['log'][-1]['pow']- mac2['log'][0]['pow']) < 10  :
		#print "controllo mac1log-1-mac2log0 e potenza"		
		Aggrega(mac1 , macaddr1 , mac2 , macaddr2)	
		return True;
...
def getSignature(packet):
	...
	while Dot11Elt in packet and i < ie_l:
		temp = pp[Dot11Elt]
		sig=sig+str(temp.ID)+','
		pp=temp.payload
		i = i+1
	...
	while Dot11Elt in packet and i < ie_l:
		...
		if pkt.ID==50: #ESRates
			for rate in range(0,pkt.len):
				value50=value50+ '{0:0{1}X}'.format(ord(pkt.info[rate:rate+1]),2)
			sig50='esrates:'+value50+','

		if pkt.ID==1: #Rates
			for rate in range(0,pkt.len):
				value1=value1+  '{0:0{1}X}'.format(ord(pkt.info[rate:rate+1]),2)
			sig1='rates:'+value1+','
	...
	sig=sig+sig221+sig45+sig1+sig50+sig191+sig33+sig127
	return sig
	...
	for k in SigList.keys():
		for q in SigList[k].keys():
			if SigList[k][q]['RandomMac']==True:
				...
				if abs(SigList[k][q]['log'][-1]['time']-time.time()) > 70:
					SigList[k][q]['canc']=True
...
def PacketHandler(pkt) :
    ...
    #probe request e controllo che la potenza sia > -99 => -(256-ord(pkt.notdecoded[-4:-3])=potenza=>RSSI)
		if pkt.type == 0 and pkt.subtype == 4 and -(256-ord(pkt.notdecoded[-4:-3])) > -99 :
			...
			if isRandomMac(pkt):
				print "=========================================================="
				print "TROVATO",pkt.addr2,"SN=",EstractSN(pkt.SC),"POTENZA=",-(256-ord(pkt.notdecoded[-4:-3]))
			if pkt.addr2=="88:70:8c:15:49:c9":
				conto=conto+1
				print "Trovato ", pkt.addr2,conto
			...
			if delta_reali > 0 or delta_random > 0:
				updateDb(pkt.addr2,0,delta_reali,delta_random)
			old_reali = now_reali
	...
	def run(self):
		...
		while True:
			...
			try:
				...
				if len(sql_struct)!=0:
					cursor = mydb.cursor()
					sql = "INSERT INTO logv3 (Id,macaddr,delta_reali,delta_random,Timestamp,aggregated) VALUES (%s,%s,%s,%s,%s,%s)"
					#print "=========================================="
					print "ho trovato qualcosa: lunghezza",len(sql_struct)
					val=[]
					for l in sql_struct:	
						tempval=(snifferId,l[1],l[2],l[3],l[5],l[4])
						val.append(tempval)
						'''
						try:
							val=(snifferId,l[1],l[2],l[3],l[5],l[4])
							cursor.execute(sql,val)
							#print(cursor.rowcount," record inserted")
							mydb.commit()
						except mysql.connector.IntegrityError:
							print "Chiave duplicata"
						'''
					try:
					...
					cursor = mydb.cursor()
					sql = "INSERT INTO log (Id,Mac_reali,Mac_random) VALUES (%s,%s,%s)"
					val = (snifferId,now_reali,now_random)
					cursor.execute(sql,val)
					print cursor.rowcount, "log inserted" 
					mydb.commit()
					...
					val = (snifferId,now_reali,now_random)
					cursor.execute(sql,val)
					print cursor.rowcount, "log insterted"
