...
def Aggrega( mac1 , macaddr1 , mac2 , macaddr2 ):
	if mac1['myid'] < mac2['myid']: #(mac1['log'][-1]['time'] < mac2['log'][-1]['time']):
		print "vecchio:",macaddr1,"aggregato con nuovo:",macaddr2
		stringa="\nCurrent MAC:\t"+macaddr1+"\nNew MAC:\t"+macaddr2
		f = open("report.txt", "a")
		f.write(stringa)
		f.close()
		...
		print "vecchio:",macaddr2,"aggregato con nuovo:",macaddr1
		stringa="\nCurrent MAC:\t"+macaddr2+"\nNew MAC:\t"+macaddr1
		f = open("report.txt", "a")
		f.write(stringa)
		f.close()
...
def Compara( mac1 , macaddr1 , mac2 , macaddr2 ): #mac1=>set1 mac2=>set2
	...
	if mac1['uuid'] != '' and mac1['uuid'] == mac2['uuid']:
		...
		return True;
	elif mac1['log'][-1]['seq'] > 25 and mac2['log'][0]['seq'] > 25:
	...
	elif abs(mac1['log'][0]['time']- mac2['log'][-1]['time']) > 45 and abs(mac1['log'][0]['pow']- mac2['log'][-1]['pow']) < 10  :
	#il time del primo log del mac i e il time dell'ultimo log del mac j siano distanti meno di 2 secondi e che le due potenze differiscano poco
		#print "controllo mac1log0-mac2log-1 e potenza"
		Aggrega(mac1 , macaddr1 , mac2 , macaddr2)
		return True;
	
	elif abs(mac1['log'][-1]['time']- mac2['log'][0]['time']) > 45 and abs(mac1['log'][-1]['pow']- mac2['log'][0]['pow']) < 10  :
...
def getLog(packet):
	return { 'time' : packet.time , 'pow' : packet.dBm_AntSignal , 'seq' : EstractSN(packet.SC)  };
...
def getSignature(packet):
	...
	while Dot11Elt in packet and i < ie_l:
		...
		if pkt.ID==1: #Rates
			for rate in range(0,pkt.len):
				value1=value1+  '{0:0{1}X}'.format(pkt.rates[rate],2)
			sig1='rates:'+value1+','
	...
	for k in SigList.keys():
		for q in SigList[k].keys():
			if SigList[k][q]['RandomMac']==True:
				if abs(SigList[k][q]['log'][-1]['time'] - time.time()) > 50:
					SigList[k][q]['canc'] = True
					#print "cancello random per vecchiaia",q
			else:
				if abs(SigList[k][q]['log'][-1]['time']-time.time()) > 90:
					SigList[k][q]['canc']=True
...
def PacketHandler(pkt) :
    ...
    #probe request e controllo che la potenza sia > -99 => -(256-ord(pkt.pkt.ded[-4:-3])=potenza=>RSSI)
		if pkt.type == 0 and pkt.subtype == 4 and pkt.dBm_AntSignal > -99 :
			key = getSignature(pkt)
			if isRandomMac(pkt):
				print "Trovato ", pkt.addr2, "potenza:",pkt.dBm_AntSignal,"SN:",EstractSN(pkt.SC) 
				#print pkt.show()
