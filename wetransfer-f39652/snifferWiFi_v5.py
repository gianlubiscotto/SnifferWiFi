		...
		for q in SigList[k].keys():
			if (SigList[k][q]['canc'])== True:
				...
				if SigList[k][q]['RandomMac']==True:
					...
					reali = -1
					random = 0
				now_reali = int(totAggrJ['MacReali'])-canc_reali
				now_random = int(totAggrJ['MacRandom'])-canc_random
				tot={'MacReali' : now_reali , 'MacRandom' : now_random}  
				updateDb(q,tot,reali,random)
				SigList[k].pop(q , None)		
				if not SigList[k]:
					SigList[k].clear()
					SigList.pop(k,None)
	...
	if pkt.addr2 is not None :
		if pkt.type == 0 and pkt.subtype == 4 and -(256-ord(pkt.notdecoded[-4:-3])) > -99 :
			if (reali != 0) or (random != 0):
				lock.acquire()
				updateDb(pkt.addr2,totAggrJ,reali,random)
				lock.release()
