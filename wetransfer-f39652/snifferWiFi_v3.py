	...
	for k in SigList.keys() :
		for q in SigList[k].keys():
			if (SigList[k][q]['canc'])== True:
				...
				updateDb(mac,totAggrJ,reale,random)
				if not SigList[k]:
					#del SigList[k]
					SigList[k].clear()
					SigList.pop(k,None)
...
def updateDb(address,totmacs,reale,random):
	global sql_struct
	global snifferId
	...
	sublist.append(reale)
	sublist.append(random)
	sublist.append(int(totmacs['MacReali']))
	sublist.append(int(totmacs['MacRandom']))
			...
			if (SigList.has_key(key)): #se la signature derivata dal modello è presente nella mia lista
				...
				if (SigList[key].has_key(pkt.addr2)):	#e ha anche associato il macaddr2 appena rilevato
					...
					if isRandomMac(pkt):
						...
						reale=1
					updateDb(pkt.addr2,totAggrJ,reale,random)
					lock.release()
				...
				flag = False
				flag = checkWrongSignature(pkt,key)
				#se flag = false aggiungo log al db
				if not flag: #se il mac è nuovo 
					print "Nuovo Mac"
					totAggrJ = ContaDetJ(SigList)
					lock.acquire()
					reale=0
					...
					print "Mac address già in memoria sotto altra signature"
		...
		while True:
			...
			try:
					...
					lock.release()
                		...
                		print("Errore invio")
			try:
				userdata = {'Id' : snifferId , 'numReali' : int(totAggrJ['MacReali'])   , 'numRandom' : int(totAggrJ['MacRandom'])}
			...
			for k in SigList:
				for q in SigList[k]:
					...
					if SigList[k][q]['RandomMac']:
						...
						reale=-1
					updateDb(q,totAggrJ,reale,random)
			#del sql_struct[:]
			del listaAp[:]
			SigList.clear()
			ApList.clear()
