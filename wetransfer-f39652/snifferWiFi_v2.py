...
#	os.system('clear')	
	...
	#os.system('clear')
	reali = int(totAggrJ['MacReali']) - old_reali
	random = int(totAggrJ['MacRandom']) - old_random
	if pkt.type == 0 and pkt.subtype == 4 and -(256-ord(pkt.notdecoded[-4:-3])) > -99 :
...
class ThreadInvio():
	...
	def run(self):
		...
		while True:
			...
			try:
					#cursor = mydb.cursor()
					#sql = "INSERT INTO Persone (snifferId, macaddr, reali, random, now_reali, now_random, time) VALUES (%s, %s, %s, %s, %s, %s, %s)"
					lock.acquire()
					#connessione al db per registrare i dati
					for l in sql_struct:	
						userdata = {'Id' : l[0] , 'now_reali' : l[4] , 'now_random' : l[5] , 'time' : l[6] , 'macaddr' : l[1] , 'reali' : l[2] , 'random' : l[3]}
						resp = requests.post('http://sniffer5terre.altervista.org/php/gestione_dativ2.php', params=userdata)
						print("Inviato\n")
					#for l in sql_struct: 
